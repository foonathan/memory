                    ...
                    std::initializer_list<std::size_t> node_sizes)
{
    ...
    for (auto count : counts)
        for (auto size : node_sizes)
        {
            ...
            auto small_alloc = make_allocator_adapter(memory_pool<small_node_pool>{size, count * size * 2});
            auto node_alloc = make_allocator_adapter(memory_pool<node_pool>{size, count * size * 2});
            auto array_alloc = make_allocator_adapter(memory_pool<array_pool>{size, count * size * 2});

            auto stack_alloc = make_allocator_adapter(memory_stack<>{count * size * 2});
