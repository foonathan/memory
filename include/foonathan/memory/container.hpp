...
namespace memory
{
    ...
    FOONATHAN_ALIAS_TEMPLATE(
        ...
        std::forward_list<T, std::scoped_allocator_adaptor<std_allocator<T, RawAllocator, Mutex>>>);
    ...
    FOONATHAN_ALIAS_TEMPLATE(multiset,
                             std::multiset<T, std::less<T>, std_allocator<T, RawAllocator, Mutex>>);
    /// \copydoc vector_scoped_alloc
    template <typename T, class RawAllocator, class Mutex = default_mutex>
    FOONATHAN_ALIAS_TEMPLATE(
    ...
    FOONATHAN_ALIAS_TEMPLATE(
        string,
        std::basic_string<char, std::char_traits<char>, std_allocator<char, RawAllocator, Mutex>>);
    /// @}
    ...
    std::priority_queue<T, Container, Compare> make_priority_queue(RawAllocator& allocator,
                                                                   Compare       comp = {})
    {
        return std::priority_queue<T, Container, Compare>{detail::move(comp), Container(allocator)};
    }
    /// @}
