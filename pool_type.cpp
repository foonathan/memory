...
void detail::deallocate(node_pool, free_memory_list &free_list, void *node) noexcept
{
    free_list.deallocate(node);
}

void detail::deallocate(array_pool, free_memory_list &free_list, void *node) noexcept
{
    free_list.deallocate_ordered(node);
}

void detail::deallocate(array_pool, free_memory_list &free_list, void *node, std::size_t n) noexcept
{
    free_list.deallocate_ordered(node, n);
}

void detail::deallocate(small_node_pool, small_free_memory_list &free_list, void *node) noexcept
{
    free_list.deallocate(node);
}
